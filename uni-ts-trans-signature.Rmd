...
layout: page
title: "한국 코로나19"
subtitle: "단변량 시계열 변환 후 예측모형 - 시간특징"
author:
- name: "이광춘"
...
<center>
![](fig/univariate-model-with-time-variable.png)
</center>
...
wkfl_fit_ets <- corona_fcst_list$model$wkfl_fit_ets
wkfl_fit_tbats <- corona_fcst_list$model$wkfl_fit_tbats
wkfl_fit_prophet <- corona_fcst_list$model$wkfl_fit_prophet
```
...
## 목표변수 - $Y$ {#corona-univariate-transformation-y}
...
## 예측변수 - $X$ {#corona-univariate-transformation-x}

날짜와 시간정보는 그 내부에 상당히 많은 정보가 포함되어 있다.
날짜로부터 여러 시간과 연관된 많은 정보를 추출하여 예측력을 높이는데 사용한다.

```{r time-signature}
history_time_feature_tbl <- history_tbl %>% 
  tk_augment_timeseries_signature(.date_var = 날짜) %>% 
  select(-year, -diff, -matches("(iso)|(xts)|(hour)|(minute)|(second)|(am.pm)")) 

history_time_feature_tbl %>% 
  glimpse()
```

## 훈련/시험 데이터 분할

```{r time-signature-split}
# splits <- history_tbl %>%
splits <- history_time_feature_tbl %>%
...
날짜와 시간정보는 그 내부에 상당히 많은 정보가 포함되어 훈련/시험 데이터를 나눌 때 관련 내용을 준비해도 좋지만,
`recipe()` 를 정의할 때 시계열 날짜 및 시간 관련 특징을 추출하여 예측력을 높이는 변수로 준비한다.
시각관련 특징이 추출되면 연속형 변수는 정규화 범주형 변수는 가변수(one-hot) 변환을 사전 수행하여 
모형 행렬(Design Matrix)을 준비한다.

```{r uni-feature-engineering}
recipe_spec <- recipes::recipe(확진자 ~ ., data = training(splits)) %>% 
    # 시계열 특징 추출
    # step_timeseries_signature(날짜) %>%
...
페이스북에서 개발한 Prophet 예측모형으로 적합시킨다.
...
mdl_glment_spec <- linear_reg(
    penalty = 0.1,
    mixture = 0.5
) %>%
    set_engine("glmnet")

wkfl_fit_glmnet <- workflow() %>%
    add_recipe(recipe_spec) %>%
    add_model(mdl_glment_spec) %>%
    fit(training(splits))
...
model_tbl <- modeltime_table(
  ...
  update_model_description(.model_id = 1, "ETS") %>% 
  update_model_description(.model_id = 2, "TBATS") %>% 
  update_model_description(.model_id = 3, "Prophet") 
  # update_model_description(.model_id = 4, "GLMNET")
...
refit_tbl <- calibration_tbl %>% 
  modeltime_refit(data = history_time_feature_tbl)
```
...
refit_tbl %>% 
  modeltime_forecast(
    ...
    actual_data = history_time_feature_tbl,
    conf_interval = 0.3
  )  %>%
  mutate(
...
corona_fcst_list <- list(
  ...
  model = list(
    ...
    # wkfl_fit_glmnet  = wkfl_fit_glmnet
  )
)
